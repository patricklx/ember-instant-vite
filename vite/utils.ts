...
if (fs.existsSync(path.join(cache, 'project.json'))) {
  console.log('loading cache')
  app = JSON.parse(fs.readFileSync(path.join(cache, 'project.json')));
  ...
  if (!didSetup) {
    ...
    try {
      ...
      config.finalize();
      // const macrobabel = config.babelPluginConfig();
      // main.options.babel.plugins.push(...config.babelPluginConfig());
    } catch (e) {
      console.log(e);
    }
  }
  ...
  fs.writeFileSync(cache + '/project.json', JSON.stringify(appJson, null, 2));
}
